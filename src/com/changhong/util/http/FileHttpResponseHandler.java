package com.changhong.util.http;
...
public class FileHttpResponseHandler extends AsyncHttpResponseHandler
{
	...
	protected void sendResponseMessage(HttpResponse response)
	{
		...
		try
		{
			...
			CHLogger.v(TAG, "totalSize: " + totalSize);
			...
			} else if (tempFile.exists())
			{
				...
				// +
				// "-");
				CHLogger.v(TAG, "yahooo: "
						+ response.getEntity().getContentLength());
				previousFileSize = tempFile.length();

				CHLogger.v(TAG, "File is not complete, download now.");
				CHLogger.v(TAG, "File length:" + tempFile.length()
						+ " totalSize:" + totalSize);
			...
			CHLogger.v(TAG, "Download completed successfully.");
			result = bytesCopied;
	...
	public int copy(InputStream input, RandomAccessFile out) throws IOException
	{
		...
		CHLogger.v(TAG, "length" + out.length());
		int count = 0, n = 0;
